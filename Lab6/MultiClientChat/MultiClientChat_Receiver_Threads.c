...
void clientchildfunction()
{
	int s_server_child = clientdatapass.clients[clientdatapass.client_index].s_server;
	DisplayConnectedClients(clientdatapass.clients, clientdatapass.clients_size);
	/*
	...
	*/
	...
	else 
	{
		...
		int destindex = FindClient(clientdatapass.clients, clientdatapass.clients_size, dest_name);
		...
		else 
		{
			...
			while(end == 0)
			{
				...
				{
					printf("%s\n", text);
					send(clientdatapass.clients[destindex].s_server, name, sizeof(name), 0);
					send(clientdatapass.clients[destindex].s_server, text, sizeof(text), 0);
				}
			}
		}
	}
		...
		else 
		{
			...
			{ 
				...
				if(accepterror == 0)
				{
					...
					struct ClientDataPass clientdatapass;

					int s_server_child = s_server;
					int clientindex = clientdatapass.clients_size;

					clientdatapass.clients[clientindex].s_server = s_server_child;
					strcpy(clientdatapass.clients[clientindex].ipaddr, clientip);
					...
					strcpy(clientdatapass.clients[clientindex].name, name);
					//strcpy(clientdatapass.clients[clientindex].ipaddr, ipaddr);
					
					strcpy(clientdatapass.clients[clientindex].port_str, port_str);
					clientdatapass.clients_size++;

					pthread_t tid;
					pthread_attr_t attr;
					pthread_attr_init(&attr);
					pthread_create(&tid, &attr, clientchildfunction, clientdatapass);
