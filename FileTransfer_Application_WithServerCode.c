...
int main()
{
	...
	{
		// Child for receiving
		c_socket_recv = ServerCreate(port);
		...
		while(exit == 0)
		{
			...
			/*
			int * msgport;
			recv(c_socket_recv, msgport, sizeof(msgport), 0);
			if(port == *msgport) continue;*/
	...
	{
		// Parent for sending
		c_socket_send = ClientCreate(port, 1, "");
		...
		while(exit == 0)
		{
			...
			int * msgport;
			msgport = &port;
			send(c_socket_send, msgport, sizeof(msgport), 0);*/
